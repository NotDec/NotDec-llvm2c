...
![sample-graph1](StructuralAnalysis/graph1.drawio.svg)
...
这里的操作对应代码中 `reduceIncSwitch` 函数。一个最经典的switch结构如下图所示。我们总结出如下几个规律：

![](switch1.drawio.png)

1. 有个固定的follow块，包含switch结束时的后继语句，也是switch中break语句的目标。
2. 每个case仅有Head一个前驱。后继节点最好仅有一个后继，或者没有任何后继。

如果case块包含多个顺序连接的基本块，或者包含复杂的If-Else结构怎么办？这时我们期望其他的结构恢复模式先匹配并处理这些情况，并将其简化为仅有一个基本块的情况，然后再应用我们这个模式来匹配。

匹配的时候我们需要做什么？

1. 如果发现有基本块的后继大于2，则尝试匹配switch语句的模式。
2. 确保switch块符合这个模式。
   1. 找出follow块。或者，所有的基本块都是Tail块（没有后继）。
   2. 每个case块的前驱有且仅有switch Head。
3. 创建Switch语句(`clang::SwitchStmt`)，并为每个Case块创建Case标签。
4. 维护CFG的边，将Head块简化为只有一个Follow作为后继块。
   1. 移除所有的Case块和相关的边。
   2. 将Switch作为一个普通的语句插入Head块。
   3. 添加一条从Head块到Follow块的边。

直接实现这些逻辑之后，会出现报错，发现有一些Case块被移除了多次，这是怎么回事？原来有一些不同Case值会跳转到同一个基本块。我们不能按照Case去遍历，而是应该按照Case块去遍历。

最好先整理一遍，根据不同的跳转目标，把case分类放好，一个代码块可能对应多个Case值。我们可以创建一个Map，从基本块映射到一组Case值。然后依次处理每个块，先创建一组Case标签，然后再把基本块内的语句放在后面。

<!-- 添加一个图，不同的case值跳转到同一个块，然后根据不同的跳转目标，把case分类放好 -->

在Clang的语法树中，`CaseStmt` 主要有两个成员，表示Case值的表达式，以及Case块内部的语句指针。如果有多个语句，我们可以创建代表大括号的CompoundStmt，组合成单个语句。这里和Goto标签也有点像，都有一个SubStmt。

如果我们想要把多个标签叠起来，则对应什么样的语法树呢？使用Clang把AST “dump” 打印出来，我们可以发现它们呈嵌套的结构。

<!-- 添加一个图，左边展示叠起来的Case，右边显示嵌套结构 -->

但是还有一些其他的情况需要处理，导致我们这里处理的代码又变得更复杂了：

- 如果某个Case直接跳转到follow块怎么办？此时我们创建一个仅有break的语句的Case标签。
- default标签的处理
  - 如果某个基本块也是default标签的目标，我们则也把default标签叠加在前面。
  - 如果default标签的目标是单独的一个基本块的话，我们需要和处理普通的Case一样，处理它。
  - 如果default标签的目标直接就是follow块的话，我们可以直接省略这个default标签。因为根据C语言switch语句的定义，如果值没有匹配任何case，且没有default的标签的话，则直接跳过switch块执行后面的内容。

...
void testLLVMFunctionCFGBuilderRecordsRangeConditionCompare() {
  ...
  llvm::BasicBlock *UnsignedCheck =
      llvm::BasicBlock::Create(Context, "unsigned_check", F);
  llvm::BasicBlock *UnsignedCase =
      llvm::BasicBlock::Create(Context, "unsigned_case", F);
  llvm::BasicBlock *Else = llvm::BasicBlock::Create(Context, "else", F);
  ...
  Builder.CreateCondBr(CmpGe, Ge, UnsignedCheck);
  Builder.SetInsertPoint(Ge);
  Builder.CreateRet(llvm::ConstantInt::get(I32Ty, 2));
  Builder.SetInsertPoint(UnsignedCheck);
  llvm::Value *CmpUgt =
      Builder.CreateICmpUGT(X, llvm::ConstantInt::get(I32Ty, 250), "cmp_ugt");
  Builder.CreateCondBr(CmpUgt, UnsignedCase, Else);
  Builder.SetInsertPoint(UnsignedCase);
  Builder.CreateRet(llvm::ConstantInt::get(I32Ty, 3));
  Builder.SetInsertPoint(Else);
  Builder.CreateRet(llvm::ConstantInt::get(I32Ty, 0));
  ...
  const CFGBlock *UnsignedBlock = Cfg.getBlock(4);
  assert(UnsignedBlock != nullptr);
  Compare = Cfg.conditionCompare(UnsignedBlock->Condition);
  assert(Compare.has_value());
  assert(Compare->Kind == ConditionCompareKind::GreaterThan);
  assert(Compare->TrueTargetIndex == 1);
  assert(Compare->EqualTargetIndex == 1);
  assert(Payloads[Compare->ComparedValue.Id] == "x");
  assert(Payloads[Compare->ConstantValue.Id] == "250");
  assert(Compare->HasIntegerValue);
  assert(!Compare->SignedPredicate);
  assert(Compare->SignedIntegerValue == 250);
...
void testStructuredCFGDuplicateCopiedDephicationEdgeAfterOriginalRemoval() {
  StructuredCFG Cfg;
  Cfg.addBlock(block(1, {4}));
  ...
  std::optional<DuplicatedRegion> FirstCopy = Cfg.duplicateRegion({1, 4, 3});
  assert(FirstCopy.has_value());
  BlockId CopyIncoming1 = FirstCopy->copyOf(1);
  BlockId CopyEdge1 = FirstCopy->copyOf(4);
  BlockId CopyMerge1 = FirstCopy->copyOf(3);
  assert(CopyIncoming1 != InvalidBlockId);
  assert(CopyEdge1 != InvalidBlockId);
  assert(CopyMerge1 != InvalidBlockId);
  assert(Cfg.dephicationVVars().size() == 2);
  VVarId CopyVVar1 = Cfg.dephicationVVars()[1].Id;

  assert(Cfg.removeBlocks({1, 4, 3}));
  assert(Cfg.getBlock(1) == nullptr);
  assert(Cfg.getBlock(4) == nullptr);
  assert(Cfg.getBlock(3) == nullptr);
  assert(Cfg.dephicationVVars()[0].Retired);
  assert(!Cfg.dephicationVVars()[1].Retired);
  assert(Cfg.dephicationIncomings().size() == 1);
  assert(Cfg.dephicationIncomings().front().IncomingBlock == CopyIncoming1);
  assert(Cfg.dephicationIncomings().front().MergeBlock == CopyMerge1);
  assert(Cfg.dephicationIncomings().front().EdgeBlock == CopyEdge1);
  assert(Cfg.dephicationIncomings().front().Target == CopyVVar1);

  std::optional<DuplicatedRegion> SecondCopy =
      Cfg.duplicateRegion({CopyIncoming1, CopyEdge1, CopyMerge1});
  assert(SecondCopy.has_value());
  BlockId CopyIncoming2 = SecondCopy->copyOf(CopyIncoming1);
  BlockId CopyEdge2 = SecondCopy->copyOf(CopyEdge1);
  BlockId CopyMerge2 = SecondCopy->copyOf(CopyMerge1);
  assert(CopyIncoming2 != InvalidBlockId);
  assert(CopyEdge2 != InvalidBlockId);
  assert(CopyMerge2 != InvalidBlockId);
  assert(Cfg.dephicationVVars().size() == 3);
  VVarId CopyVVar2 = Cfg.dephicationVVars()[2].Id;
  assert(Cfg.dephicationVVars()[2].SourceId == VVar);
  assert(Cfg.dephicationVVars()[2].MergeBlock == CopyMerge2);
  assert(!Cfg.dephicationVVars()[2].Retired);

  const std::vector<DephicationIncoming> &Incomings =
      Cfg.dephicationIncomings();
  assert(Incomings.size() == 2);
  assert(Incomings[1].IncomingBlock == CopyIncoming2);
  assert(Incomings[1].MergeBlock == CopyMerge2);
  assert(Incomings[1].EdgeBlock == CopyEdge2);
  assert(Incomings[1].SourceIncomingBlock == CopyIncoming1);
  assert(Incomings[1].SourceMergeBlock == CopyMerge1);
  assert(Incomings[1].SourceEdgeBlock == CopyEdge1);
  assert(Incomings[1].SourceTarget == CopyVVar1);
  assert(Incomings[1].Target == CopyVVar2);

  DephicationEdgeContext CopyEdgeContext = Cfg.dephicationEdgeContext(CopyEdge2);
  assert(CopyEdgeContext.Incomings.size() == 1);
  assert(CopyEdgeContext.VVarCopies.size() == 1);
  assert(CopyEdgeContext.VVarCopies.begin()->first == CopyVVar1);
  assert(CopyEdgeContext.VVarCopies.begin()->second == CopyVVar2);

  DephicationEdgeContext CopyMergeContext =
      Cfg.dephicationBlockContext(CopyMerge2);
  assert(CopyMergeContext.VVars.size() == 1);
  assert(CopyMergeContext.VVars.front().Id == CopyVVar2);
  assert(CopyMergeContext.VVarCopies.size() == 1);
  assert(CopyMergeContext.VVarCopies.begin()->first == VVar);
  assert(CopyMergeContext.VVarCopies.begin()->second == CopyVVar2);

  bool SawCopiedIncoming = false;
  bool SawCopiedMerge = false;
  Cfg.setPayloadMaterializeHook(
      [&](const PayloadMaterializeContext &Context,
          PayloadMaterializeKind Kind, PayloadRef Payload,
          std::size_t) -> std::optional<PayloadRef> {
        if (Context.CopyBlock == CopyEdge2 &&
            Kind == PayloadMaterializeKind::DephicationAssignment) {
          assert(Context.CurrentDephicationIncoming.has_value());
          assert(Context.CurrentDephicationIncoming->SourceTarget == CopyVVar1);
          assert(Context.CurrentDephicationIncoming->Target == CopyVVar2);
          assert(Context.DephicationVVarCopies.size() == 1);
          assert(Context.DephicationVVarCopies.begin()->first == CopyVVar1);
          assert(Context.DephicationVVarCopies.begin()->second == CopyVVar2);
          SawCopiedIncoming = true;
          return PayloadRef{Payload.Id + 1000};
        }

        if (Context.CopyBlock == CopyMerge2 &&
            Kind == PayloadMaterializeKind::Statement) {
          assert(Context.DephicationVVars.size() == 1);
          assert(Context.DephicationVVars.front().Id == CopyVVar2);
          assert(Context.DephicationVVarCopies.size() == 1);
          assert(Context.DephicationVVarCopies.begin()->first == VVar);
          assert(Context.DephicationVVarCopies.begin()->second == CopyVVar2);
          SawCopiedMerge = true;
  ...
  assert(Cfg.materializeBlockBody(CopyEdge2));
  assert(Cfg.materializeBlockBody(CopyMerge2));
  assert(SawCopiedIncoming);
  assert(SawCopiedMerge);
  assert(Cfg.dephicationIncomings()[1].Assignment.Id == 1040);
  assert(hasSinglePayload(Cfg.getBlock(CopyMerge2)->Statements, 2041));
}
...
void testDuplicationReverterExtractsGotoBranchCommonStatementTail() {
  StructuredCFG Cfg;
  Cfg.addBlock(branchBlock(0, {1, 2}));
...
void testDuplicationReverterKeepsBranchCommonStatementTailWithoutGotoHint() {
  ...
  assert(Cfg.getBlock(2)->Successors == std::vector<BlockId>{5});
}
...
void testReturnDuplicatorLowCopiesReturnEndNodeWithDephicationVVars() {
  StructuredCFG Cfg;
  ...
  bool SawCopiedAssignment = false;
  bool SawCopiedReturn = false;
  Cfg.setPayloadMaterializeHook(
      [&](const PayloadMaterializeContext &Context,
          PayloadMaterializeKind Kind, PayloadRef Payload,
          std::size_t) -> std::optional<PayloadRef> {
        ...
        if (Kind == PayloadMaterializeKind::DephicationAssignment) {
          SawCopiedAssignment = true;
          assert(Context.CurrentDephicationIncoming.has_value());
          assert(Context.CopyBlock == 3);
          assert(Context.CurrentDephicationIncoming->Target == CopiedVVar);
          assert(Context.CurrentDephicationIncoming->SourceTarget == VVar);
          assert(Context.CurrentDephicationIncoming->Assignment.Id == 30);
          return PayloadRef{1030};
        }

        if (Kind == PayloadMaterializeKind::Statement && Payload.Id == 20) {
          SawCopiedReturn = true;
          assert(!Context.CurrentDephicationIncoming.has_value());
          return PayloadRef{1020};
        }
  ...
  StructuringEvaluation Current;
  Current.Gotos = GotoManager::fromGotos({StructuredGoto{3, 2}});
  ...
  assert(SawCopiedAssignment);
  assert(SawCopiedReturn);
  ...
  assert(Block0 != nullptr && Block1 != nullptr && Edge0Block != nullptr &&
         Edge1Block != nullptr && OriginalRet != nullptr);
  ...
  assert(Edge1Block->Successors == std::vector<BlockId>{2});
  assert(hasSinglePayload(Edge0Block->Statements, 1030));
  assert(hasSinglePayload(Edge1Block->Statements, 31));
  assert(hasSinglePayload(OriginalRet->Statements, 20));

  const CFGBlock *CopyRet = Cfg.getBlock(Edge0Block->Successors.front());
  assert(CopyRet != nullptr);
  assert(CopyRet->Terminator == TerminatorKind::Return);
  assert(CopyRet->SourceBlock == 2);
  assert(CopyRet->BodyBlock == CopyRet->Id);
  assert(hasSinglePayload(CopyRet->Statements, 1020));

  DephicationEdgeContext OriginalContext = Cfg.dephicationBlockContext(2);
  assert(OriginalContext.VVars.size() == 1);
  assert(OriginalContext.VVars.front().Id == VVar);
  assert(OriginalContext.Incomings.empty());

  DephicationEdgeContext OriginalEdgeContext = Cfg.dephicationEdgeContext(4);
  assert(OriginalEdgeContext.Incomings.size() == 1);
  assert(OriginalEdgeContext.Incomings.front().IncomingBlock == 1);
  assert(OriginalEdgeContext.Incomings.front().MergeBlock == 2);
  assert(OriginalEdgeContext.Incomings.front().Target == VVar);
  assert(OriginalEdgeContext.Incomings.front().Assignment.Id == 31);

  DephicationEdgeContext CopyContext =
      Cfg.dephicationBlockContext(CopyRet->Id);
  assert(CopyContext.VVars.size() == 1);
  assert(CopyContext.VVars.front().Id == CopiedVVar);
  assert(CopyContext.VVarCopies.size() == 1);
  assert(CopyContext.VVarCopies.begin()->first == VVar);
  assert(CopyContext.VVarCopies.begin()->second == CopiedVVar);
  assert(CopyContext.Incomings.empty());

  DephicationEdgeContext CopyEdgeContext = Cfg.dephicationEdgeContext(3);
  assert(CopyEdgeContext.Incomings.size() == 1);
  assert(CopyEdgeContext.Incomings.front().IncomingBlock == 0);
  assert(CopyEdgeContext.Incomings.front().MergeBlock == CopyRet->Id);
  assert(CopyEdgeContext.Incomings.front().Target == CopiedVVar);
  assert(CopyEdgeContext.Incomings.front().SourceTarget == VVar);
...
void testReturnDuplicatorLowCopiesReturnEndNodeWithMultipleDephicationVVars() {
  StructuredCFG Cfg;
  Cfg.addBlock(block(0, {3}));
  Cfg.addBlock(block(1, {4}));
  ...
  Edge1.SyntheticSource = 1;
  Edge1.SyntheticTarget = 2;
  Edge1.Statements.push_back({40});
  Edge1.Statements.push_back({41});
  Cfg.addBlock(std::move(Edge1));

  VVarId XVVar = Cfg.addDephicationVVar("x", 2);
  VVarId YVVar = Cfg.addDephicationVVar("y", 2);
  Cfg.addDephicationIncoming(XVVar, 0, 2, 3, {30}, "x");
  Cfg.addDephicationIncoming(YVVar, 0, 2, 3, {31}, "y");
  Cfg.addDephicationIncoming(XVVar, 1, 2, 4, {40}, "x");
  Cfg.addDephicationIncoming(YVVar, 1, 2, 4, {41}, "y");

  std::map<BlockId, std::set<VVarId>> CopiedVVarsByEdge;
  std::map<BlockId, std::vector<PayloadId>> RewrittenAssignments;
  Cfg.setPayloadMaterializeHook(
      [&](const PayloadMaterializeContext &Context,
          PayloadMaterializeKind Kind, PayloadRef Payload,
          std::size_t) -> std::optional<PayloadRef> {
        if (Kind == PayloadMaterializeKind::DephicationAssignment) {
          assert(Context.DephicationVVarCopies.size() == 2);
          assert(Context.DephicationVVars.size() == 2);
          assert(Context.CurrentDephicationIncoming.has_value());
          assert(Context.CurrentDephicationIncoming->SourceTarget == XVVar ||
                 Context.CurrentDephicationIncoming->SourceTarget == YVVar);

          auto CopyIt = Context.DephicationVVarCopies.find(
              Context.CurrentDephicationIncoming->SourceTarget);
          assert(CopyIt != Context.DephicationVVarCopies.end());
          assert(Context.CurrentDephicationIncoming->Target == CopyIt->second);
          CopiedVVarsByEdge[Context.CopyBlock].insert(CopyIt->second);

          PayloadRef Rewritten{Payload.Id + 1000};
          RewrittenAssignments[Context.CopyBlock].push_back(Rewritten.Id);
          return Rewritten;
        }
  ...
  assert(Changed);
  assert(Cfg.getBlock(2) == nullptr);
  assert(CopiedVVarsByEdge.size() == 2);
  assert(RewrittenAssignments.size() == 2);

  const CFGBlock *Edge0Block = Cfg.getBlock(3);
  const CFGBlock *Edge1Block = Cfg.getBlock(4);
  assert(Edge0Block != nullptr && Edge1Block != nullptr);
  assert(Edge0Block->Successors.size() == 1);
  assert(Edge1Block->Successors.size() == 1);
  assert(Edge0Block->Successors.front() != 2);
  assert(Edge1Block->Successors.front() != 2);
  assert(Edge0Block->Successors.front() != Edge1Block->Successors.front());
  assert(RewrittenAssignments[3] == std::vector<PayloadId>({1030, 1031}));
  assert(RewrittenAssignments[4] == std::vector<PayloadId>({1040, 1041}));
  assert(Edge0Block->Statements.size() == 2);
  assert(Edge0Block->Statements[0].Id == 1030);
  assert(Edge0Block->Statements[1].Id == 1031);
  assert(Edge1Block->Statements.size() == 2);
  assert(Edge1Block->Statements[0].Id == 1040);
  assert(Edge1Block->Statements[1].Id == 1041);
  ...
  const std::vector<DephicationVVar> &AllVVars = Cfg.dephicationVVars();
  assert(AllVVars.size() == 6);
  assert(AllVVars[XVVar].Retired);
  assert(AllVVars[YVVar].Retired);
  for (const auto &[Edge, VVars] : CopiedVVarsByEdge) {
    (void)Edge;
    assert(VVars.size() == 2);
    for (VVarId CopyVVar : VVars) {
      assert(CopyVVar < AllVVars.size());
      assert(!AllVVars[CopyVVar].Retired);
      assert(AllVVars[CopyVVar].SourceId == XVVar ||
             AllVVars[CopyVVar].SourceId == YVVar);
    }
  }

  DephicationEdgeContext CopyEdge0 = Cfg.dephicationEdgeContext(3);
  DephicationEdgeContext CopyEdge1 = Cfg.dephicationEdgeContext(4);
  assert(CopyEdge0.Incomings.size() == 2);
  assert(CopyEdge1.Incomings.size() == 2);
  assert(CopyEdge0.VVarCopies.size() == 2);
  assert(CopyEdge1.VVarCopies.size() == 2);
  assert(CopyEdge0.Incomings[0].MergeBlock == CopyRet0->Id);
  assert(CopyEdge0.Incomings[1].MergeBlock == CopyRet0->Id);
  assert(CopyEdge1.Incomings[0].MergeBlock == CopyRet1->Id);
  assert(CopyEdge1.Incomings[1].MergeBlock == CopyRet1->Id);
  assert(CopyEdge0.Incomings[0].Assignment.Id == 1030);
  assert(CopyEdge0.Incomings[1].Assignment.Id == 1031);
  assert(CopyEdge1.Incomings[0].Assignment.Id == 1040);
  assert(CopyEdge1.Incomings[1].Assignment.Id == 1041);
}
...
void testReturnDuplicatorLowCopiesNestedReturnRegion() {
  StructuredCFG Cfg;
  Cfg.addBlock(block(0, {2}));
  Cfg.addBlock(block(1, {2}));

  CFGBlock Head = branchBlock(2, {3, 4});
  Head.Condition = {50};
  Head.Statements.push_back({20});
  Cfg.addBlock(std::move(Head));

  CFGBlock LeftTail = block(3, {5});
  LeftTail.Statements.push_back({30});
  Cfg.addBlock(std::move(LeftTail));

  CFGBlock RightRet = block(4, {});
  RightRet.Terminator = TerminatorKind::Return;
  RightRet.Statements.push_back({40});
  Cfg.addBlock(std::move(RightRet));

  CFGBlock LeftBranch = branchBlock(5, {6, 7});
  LeftBranch.Condition = {51};
  LeftBranch.Statements.push_back({31});
  Cfg.addBlock(std::move(LeftBranch));

  CFGBlock LeftRet = block(6, {});
  LeftRet.Terminator = TerminatorKind::Return;
  LeftRet.Statements.push_back({60});
  Cfg.addBlock(std::move(LeftRet));

  CFGBlock LeftRet2 = block(7, {});
  LeftRet2.Terminator = TerminatorKind::Return;
  LeftRet2.Statements.push_back({70});
  Cfg.addBlock(std::move(LeftRet2));
  ...
  assert(Changed);
  for (BlockId Original : {2, 3, 4, 5, 6, 7}) {
  ...
  assert(CopyHead0->Condition.Id == 50);
  assert(CopyHead1->Condition.Id == 50);
  assert(CopyHead0->Successors.size() == 2);
  assert(CopyHead1->Successors.size() == 2);

  const CFGBlock *CopyLeftTail0 = Cfg.getBlock(CopyHead0->Successors[0]);
  const CFGBlock *CopyRightRet0 = Cfg.getBlock(CopyHead0->Successors[1]);
  const CFGBlock *CopyLeftTail1 = Cfg.getBlock(CopyHead1->Successors[0]);
  const CFGBlock *CopyRightRet1 = Cfg.getBlock(CopyHead1->Successors[1]);
  assert(CopyLeftTail0 != nullptr && CopyRightRet0 != nullptr);
  assert(CopyLeftTail1 != nullptr && CopyRightRet1 != nullptr);
  assert(hasSinglePayload(CopyLeftTail0->Statements, 30));
  assert(hasSinglePayload(CopyLeftTail1->Statements, 30));
  assert(CopyRightRet0->Terminator == TerminatorKind::Return);
  assert(CopyRightRet1->Terminator == TerminatorKind::Return);
  assert(hasSinglePayload(CopyRightRet0->Statements, 40));
  assert(hasSinglePayload(CopyRightRet1->Statements, 40));

  const CFGBlock *CopyLeftBranch0 = Cfg.getBlock(CopyLeftTail0->Successors.front());
  const CFGBlock *CopyLeftBranch1 = Cfg.getBlock(CopyLeftTail1->Successors.front());
  assert(CopyLeftBranch0 != nullptr && CopyLeftBranch1 != nullptr);
  assert(CopyLeftBranch0->Terminator == TerminatorKind::Branch);
  assert(CopyLeftBranch1->Terminator == TerminatorKind::Branch);
  assert(hasSinglePayload(CopyLeftBranch0->Statements, 31));
  assert(hasSinglePayload(CopyLeftBranch1->Statements, 31));
  assert(CopyLeftBranch0->Condition.Id == 51);
  assert(CopyLeftBranch1->Condition.Id == 51);

  const CFGBlock *CopyLeftRet0 = Cfg.getBlock(CopyLeftBranch0->Successors[0]);
  const CFGBlock *CopyLeftRet1 = Cfg.getBlock(CopyLeftBranch1->Successors[0]);
  const CFGBlock *CopyLeftRet20 =
      Cfg.getBlock(CopyLeftBranch0->Successors[1]);
  const CFGBlock *CopyLeftRet21 =
      Cfg.getBlock(CopyLeftBranch1->Successors[1]);
  assert(CopyLeftRet0 != nullptr && CopyLeftRet1 != nullptr);
  assert(CopyLeftRet20 != nullptr && CopyLeftRet21 != nullptr);
  assert(CopyLeftRet0->Terminator == TerminatorKind::Return);
  assert(CopyLeftRet1->Terminator == TerminatorKind::Return);
  assert(CopyLeftRet20->Terminator == TerminatorKind::Return);
  assert(CopyLeftRet21->Terminator == TerminatorKind::Return);
  assert(hasSinglePayload(CopyLeftRet0->Statements, 60));
  assert(hasSinglePayload(CopyLeftRet1->Statements, 60));
  assert(hasSinglePayload(CopyLeftRet20->Statements, 70));
  assert(hasSinglePayload(CopyLeftRet21->Statements, 70));
}

void testReturnDuplicatorLowCopiesDiamondReturnRegionWithGroupedPredecessorRewrite() {
  StructuredCFG Cfg;
  Cfg.addBlock(branchBlock(0, {1, 2}));
  Cfg.addBlock(block(1, {2}));
  ...
  CFGBlock LeftMid = block(3, {5});
  LeftMid.Statements.push_back({30});
  Cfg.addBlock(std::move(LeftMid));

  CFGBlock RightMid = block(4, {6});
  RightMid.Statements.push_back({40});
  Cfg.addBlock(std::move(RightMid));

  CFGBlock LeftJoin = block(5, {7});
  LeftJoin.Statements.push_back({50});
  Cfg.addBlock(std::move(LeftJoin));

  CFGBlock RightJoin = block(6, {7});
  RightJoin.Statements.push_back({60});
  Cfg.addBlock(std::move(RightJoin));

  CFGBlock Ret = block(7, {});
  Ret.Terminator = TerminatorKind::Return;
  Ret.Statements.push_back({70});
  Cfg.addBlock(std::move(Ret));

  std::vector<std::vector<BlockId>> SeenGroups;
  Cfg.setPayloadMaterializeHook(
      [&](const PayloadMaterializeContext &Context,
          PayloadMaterializeKind Kind, PayloadRef Payload,
          std::size_t) -> std::optional<PayloadRef> {
        ...
        if (Context.OriginalPredecessors == std::vector<BlockId>({0, 1})) {
          SeenGroups.push_back(Context.OriginalPredecessors);
          assert(Context.NewPredecessors == std::vector<BlockId>({0, 1}));
          return PayloadRef{Payload.Id + 1000};
        }

        if (Context.OriginalPredecessors == std::vector<BlockId>({5, 6})) {
          SeenGroups.push_back(Context.OriginalPredecessors);
          assert(Context.NewPredecessors.size() == 2);
          assert(Context.NewPredecessors[0] != InvalidBlockId);
          assert(Context.NewPredecessors[1] != InvalidBlockId);
          return PayloadRef{Payload.Id + 2000};
        }
  ...
  assert(Changed);
  assert(SeenGroups.size() == 2);
  assert((SeenGroups[0] == std::vector<BlockId>({0, 1}) ||
          SeenGroups[1] == std::vector<BlockId>({0, 1})));
  assert((SeenGroups[0] == std::vector<BlockId>({5, 6}) ||
          SeenGroups[1] == std::vector<BlockId>({5, 6})));
  assert(Cfg.getBlock(2) == nullptr);
  assert(Cfg.getBlock(3) == nullptr);
  assert(Cfg.getBlock(4) == nullptr);
  assert(Cfg.getBlock(5) == nullptr);
  ...
  assert(Block0 != nullptr && Block1 != nullptr);
  assert(Block0->Successors.size() == 2);
  assert(Block0->Successors.front() == 1 || Block0->Successors.back() == 1);

  BlockId CopyHeadId = Block0->Successors.front() == 1 ? Block0->Successors.back()
                                                       : Block0->Successors.front();
  assert(CopyHeadId != InvalidBlockId);
  const CFGBlock *CopyHead = Cfg.getBlock(CopyHeadId);
  assert(CopyHead != nullptr);
  assert(CopyHead->Terminator == TerminatorKind::Branch);
  assert(CopyHead->BodyBlock == CopyHead->Id);
  assert(CopyHead->Condition.Id == 22);
  assert(hasSinglePayload(CopyHead->Statements, 1020));

  assert(Block1->Successors == std::vector<BlockId>{CopyHeadId});
  assert(CopyHead->Successors.size() == 2);

  const CFGBlock *CopyLeftMid = Cfg.getBlock(CopyHead->Successors[0]);
  const CFGBlock *CopyRightMid = Cfg.getBlock(CopyHead->Successors[1]);
  assert(CopyLeftMid != nullptr && CopyRightMid != nullptr);
  assert(hasSinglePayload(CopyLeftMid->Statements, 30));
  assert(hasSinglePayload(CopyRightMid->Statements, 40));

  const CFGBlock *CopyLeftJoin = Cfg.getBlock(CopyLeftMid->Successors.front());
  const CFGBlock *CopyRightJoin =
      Cfg.getBlock(CopyRightMid->Successors.front());
  assert(CopyLeftJoin != nullptr && CopyRightJoin != nullptr);
  assert(hasSinglePayload(CopyLeftJoin->Statements, 50));
  assert(hasSinglePayload(CopyRightJoin->Statements, 60));

  const CFGBlock *CopyRet = Cfg.getBlock(CopyLeftJoin->Successors.front());
  assert(CopyRet != nullptr);
  assert(CopyRet == Cfg.getBlock(CopyRightJoin->Successors.front()));
  assert(CopyRet->Terminator == TerminatorKind::Return);
  assert(CopyRet->BodyBlock == CopyRet->Id);
...
void testReturnDuplicatorLowCopiesJoinedDiamondReturnRegion() {
  StructuredCFG Cfg;
  Cfg.addBlock(block(0, {2}));
  Cfg.addBlock(block(1, {2}));
  ...
  CFGBlock Left = block(3, {5});
  Left.Statements.push_back({30});
  Cfg.addBlock(std::move(Left));

  CFGBlock Right = block(4, {5});
  Right.Statements.push_back({40});
  Cfg.addBlock(std::move(Right));

  CFGBlock Join = block(5, {6});
  Join.Statements.push_back({50});
  Cfg.addBlock(std::move(Join));

  CFGBlock Ret = block(6, {});
  Ret.Terminator = TerminatorKind::Return;
  Ret.Statements.push_back({60});
  Cfg.addBlock(std::move(Ret));
  ...
  auto CheckCopy = [&](const CFGBlock *CopyHead) {
    assert(CopyHead != nullptr);
    assert(CopyHead->SourceBlock == 2);
    assert(CopyHead->Terminator == TerminatorKind::Branch);
    assert(CopyHead->Condition.Id == 20);
    assert(hasSinglePayload(CopyHead->Statements, 21));
    assert(CopyHead->Successors.size() == 2);

    const CFGBlock *CopyLeft = Cfg.getBlock(CopyHead->Successors[0]);
    const CFGBlock *CopyRight = Cfg.getBlock(CopyHead->Successors[1]);
    assert(CopyLeft != nullptr && CopyRight != nullptr);
    assert(CopyLeft->SourceBlock == 3);
    assert(CopyRight->SourceBlock == 4);
    assert(hasSinglePayload(CopyLeft->Statements, 30));
    assert(hasSinglePayload(CopyRight->Statements, 40));
    ...
    assert(CopyJoin->SourceBlock == 5);
    assert(hasSinglePayload(CopyJoin->Statements, 50));

    const CFGBlock *CopyRet = Cfg.getBlock(CopyJoin->Successors.front());
    assert(CopyRet != nullptr);
    assert(CopyRet->SourceBlock == 6);
    assert(CopyRet->Terminator == TerminatorKind::Return);
    assert(hasSinglePayload(CopyRet->Statements, 60));
  };

  CheckCopy(Cfg.getBlock(Block0->Successors.front()));
  CheckCopy(Cfg.getBlock(Block1->Successors.front()));
}

void testReturnDuplicatorLowCopiesJoinedDiamondWithDirectJoinSide() {
  StructuredCFG Cfg;
  Cfg.addBlock(block(0, {2}));
  Cfg.addBlock(block(1, {2}));
  ...
  auto CheckCopy = [&](const CFGBlock *CopyHead) {
    ...
    const CFGBlock *CopyLeft = nullptr;
    const CFGBlock *CopyJoin = nullptr;
    for (BlockId Succ : CopyHead->Successors) {
      const CFGBlock *SuccBlock = Cfg.getBlock(Succ);
      assert(SuccBlock != nullptr);
      if (SuccBlock->SourceBlock == 3) {
    ...
    assert(CopyLeft != nullptr && CopyJoin != nullptr);
    assert(hasSinglePayload(CopyLeft->Statements, 30));
    assert(CopyLeft->Successors == std::vector<BlockId>{CopyJoin->Id});
    assert(hasSinglePayload(CopyJoin->Statements, 50));
...
void testReturnDuplicatorLowSkipsAllDirectJoinedDiamond() {
  StructuredCFG Cfg;
  Cfg.addBlock(block(0, {2}));
  Cfg.addBlock(block(1, {2}));

  CFGBlock Head = branchBlock(2, {5, 5});
  Head.Condition = {20};
  Head.Statements.push_back({21});
  Cfg.addBlock(std::move(Head));
...
void testReturnDuplicatorLowCopiesPrefixedDiamondReturnRegion() {
  ...
  CFGBlock Prefix = block(2, {3});
  Prefix.Statements.push_back({20});
  Cfg.addBlock(std::move(Prefix));

  CFGBlock Head = branchBlock(3, {4, 5});
  Head.Condition = {30};
  Head.Statements.push_back({31});
  Cfg.addBlock(std::move(Head));

  CFGBlock Left = block(4, {6});
  Left.Statements.push_back({40});
  Cfg.addBlock(std::move(Left));

  CFGBlock Right = block(5, {7});
  Right.Statements.push_back({50});
  Cfg.addBlock(std::move(Right));

  CFGBlock LeftJoin = block(6, {8});
  LeftJoin.Statements.push_back({60});
  Cfg.addBlock(std::move(LeftJoin));

  CFGBlock RightJoin = block(7, {8});
  RightJoin.Statements.push_back({70});
  Cfg.addBlock(std::move(RightJoin));

  CFGBlock Ret = block(8, {});
  Ret.Terminator = TerminatorKind::Return;
  Ret.Statements.push_back({80});
  Cfg.addBlock(std::move(Ret));
  ...
  assert(Changed);
  for (BlockId Original : {2, 3, 4, 5, 6, 7, 8}) {
    assert(Cfg.getBlock(Original) != nullptr);
  }
  ...
  const CFGBlock *CopyPrefix = Cfg.getBlock(Block0->Successors.front());
  assert(CopyPrefix != nullptr);
  assert(CopyPrefix->SourceBlock == 2);
  assert(CopyPrefix->BodyBlock == CopyPrefix->Id);
  assert(CopyPrefix->Successors.size() == 1);
  assert(hasSinglePayload(CopyPrefix->Statements, 20));

  const CFGBlock *CopyHead = Cfg.getBlock(CopyPrefix->Successors.front());
  assert(CopyHead != nullptr);
  assert(CopyHead->SourceBlock == 3);
  assert(CopyHead->Terminator == TerminatorKind::Branch);
  assert(CopyHead->Condition.Id == 30);
  assert(hasSinglePayload(CopyHead->Statements, 31));
  assert(CopyHead->Successors.size() == 2);

  const CFGBlock *CopyLeft = Cfg.getBlock(CopyHead->Successors[0]);
  const CFGBlock *CopyRight = Cfg.getBlock(CopyHead->Successors[1]);
  assert(CopyLeft != nullptr && CopyRight != nullptr);
  assert(hasSinglePayload(CopyLeft->Statements, 40));
  assert(hasSinglePayload(CopyRight->Statements, 50));

  const CFGBlock *CopyLeftJoin = Cfg.getBlock(CopyLeft->Successors.front());
  const CFGBlock *CopyRightJoin = Cfg.getBlock(CopyRight->Successors.front());
  assert(CopyLeftJoin != nullptr && CopyRightJoin != nullptr);
  assert(hasSinglePayload(CopyLeftJoin->Statements, 60));
  assert(hasSinglePayload(CopyRightJoin->Statements, 70));

  const CFGBlock *CopyRet = Cfg.getBlock(CopyLeftJoin->Successors.front());
  assert(CopyRet != nullptr);
  assert(CopyRet == Cfg.getBlock(CopyRightJoin->Successors.front()));
  assert(CopyRet->Terminator == TerminatorKind::Return);
  assert(hasSinglePayload(CopyRet->Statements, 80));
}

void testReturnDuplicatorLowCopiesBranchPrefixedDiamondReturnRegion() {
  StructuredCFG Cfg;
  Cfg.addBlock(block(0, {2}));
  Cfg.addBlock(block(1, {2}));

  CFGBlock Outer = branchBlock(2, {3, 9});
  Outer.Condition = {20};
  ...
  CFGBlock DiamondRet = block(8, {});
  DiamondRet.Terminator = TerminatorKind::Return;
  DiamondRet.Statements.push_back({80});
  Cfg.addBlock(std::move(DiamondRet));

  CFGBlock OtherRet = block(9, {});
  OtherRet.Terminator = TerminatorKind::Return;
  OtherRet.Statements.push_back({90});
  Cfg.addBlock(std::move(OtherRet));
  ...
  const CFGBlock *CopyHead = Cfg.getBlock(CopyOuter->Successors[0]);
  const CFGBlock *CopyOtherRet = Cfg.getBlock(CopyOuter->Successors[1]);
  assert(CopyHead != nullptr && CopyOtherRet != nullptr);
  assert(CopyHead->SourceBlock == 3);
  assert(CopyHead->Terminator == TerminatorKind::Branch);
  assert(CopyHead->Condition.Id == 30);
  assert(hasSinglePayload(CopyHead->Statements, 31));
  assert(CopyOtherRet->SourceBlock == 9);
  assert(CopyOtherRet->Terminator == TerminatorKind::Return);
  assert(hasSinglePayload(CopyOtherRet->Statements, 90));
  ...
  const CFGBlock *CopyDiamondRet =
      Cfg.getBlock(CopyLeftJoin->Successors.front());
  assert(CopyDiamondRet != nullptr);
  assert(CopyDiamondRet == Cfg.getBlock(CopyRightJoin->Successors.front()));
  assert(CopyDiamondRet->Terminator == TerminatorKind::Return);
  assert(hasSinglePayload(CopyDiamondRet->Statements, 80));
}

void testReturnDuplicatorLowCopiesBranchWithDiamondReturnTail() {
  StructuredCFG Cfg;
  Cfg.addBlock(block(0, {2}));
  Cfg.addBlock(block(1, {2}));
  ...
  CFGBlock DiamondHead = branchBlock(5, {6, 7});
  DiamondHead.Condition = {50};
  DiamondHead.Statements.push_back({51});
  Cfg.addBlock(std::move(DiamondHead));

  CFGBlock Left = block(6, {8});
  Left.Statements.push_back({60});
  Cfg.addBlock(std::move(Left));

  CFGBlock Right = block(7, {9});
  Right.Statements.push_back({70});
  Cfg.addBlock(std::move(Right));

  CFGBlock LeftJoin = block(8, {10});
  LeftJoin.Statements.push_back({80});
  Cfg.addBlock(std::move(LeftJoin));

  CFGBlock RightJoin = block(9, {10});
  RightJoin.Statements.push_back({90});
  Cfg.addBlock(std::move(RightJoin));

  CFGBlock DiamondRet = block(10, {});
  DiamondRet.Terminator = TerminatorKind::Return;
  DiamondRet.Statements.push_back({100});
  Cfg.addBlock(std::move(DiamondRet));
  ...
  assert(Changed);
  for (BlockId Original : {2, 3, 4, 5, 6, 7, 8, 9, 10}) {
    assert(Cfg.getBlock(Original) != nullptr);
  }
  ...
  const CFGBlock *CopyDiamondHead = Cfg.getBlock(CopyOuter->Successors[1]);
  assert(CopyPlainTail != nullptr && CopyDiamondHead != nullptr);
  assert(CopyPlainTail->SourceBlock == 3);
  assert(CopyDiamondHead->SourceBlock == 5);
  assert(CopyDiamondHead->Terminator == TerminatorKind::Branch);
  assert(CopyDiamondHead->Condition.Id == 50);
  assert(hasSinglePayload(CopyPlainTail->Statements, 30));
  assert(hasSinglePayload(CopyDiamondHead->Statements, 51));
  ...
  const CFGBlock *CopyLeft = Cfg.getBlock(CopyDiamondHead->Successors[0]);
  const CFGBlock *CopyRight = Cfg.getBlock(CopyDiamondHead->Successors[1]);
  assert(CopyLeft != nullptr && CopyRight != nullptr);
  ...
  const CFGBlock *CopyRightJoin = Cfg.getBlock(CopyRight->Successors.front());
  assert(CopyLeftJoin != nullptr && CopyRightJoin != nullptr);
  assert(hasSinglePayload(CopyLeftJoin->Statements, 80));
  assert(hasSinglePayload(CopyRightJoin->Statements, 90));
...
void testReturnDuplicatorLowCopiesBranchWithSwitchReturnTail() {
  StructuredCFG Cfg;
  Cfg.addBlock(block(0, {2}));
  Cfg.addBlock(block(1, {2}));
  ...
  CFGBlock DefaultTail = block(6, {7});
  DefaultTail.Statements.push_back({60});
  Cfg.addBlock(std::move(DefaultTail));

  CFGBlock DefaultRet = block(7, {});
  DefaultRet.Terminator = TerminatorKind::Return;
  DefaultRet.Statements.push_back({70});
  Cfg.addBlock(std::move(DefaultRet));

  CFGBlock CaseTail = block(8, {9});
  CaseTail.Statements.push_back({80});
  Cfg.addBlock(std::move(CaseTail));

  CFGBlock CaseRet = block(9, {});
  CaseRet.Terminator = TerminatorKind::Return;
  CaseRet.Statements.push_back({90});
  ...
  const CFGBlock *CopyPlainTail = Cfg.getBlock(CopyOuter->Successors[0]);
  const CFGBlock *CopyInnerSwitch = Cfg.getBlock(CopyOuter->Successors[1]);
  assert(CopyPlainTail != nullptr && CopyInnerSwitch != nullptr);
  assert(CopyPlainTail->SourceBlock == 3);
  ...
  const CFGBlock *CopyCaseTail =
      Cfg.getBlock(CopyInnerSwitch->Cases.front().Target);
  assert(CopyDefaultTail != nullptr && CopyCaseTail != nullptr);
  assert(CopyDefaultTail->SourceBlock == 6);
  assert(CopyCaseTail->SourceBlock == 8);
  assert(hasSinglePayload(CopyDefaultTail->Statements, 60));
  assert(hasSinglePayload(CopyCaseTail->Statements, 80));

  const CFGBlock *CopyDefaultRet =
      Cfg.getBlock(CopyDefaultTail->Successors.front());
  const CFGBlock *CopyCaseRet =
      Cfg.getBlock(CopyCaseTail->Successors.front());
  assert(CopyDefaultRet != nullptr && CopyCaseRet != nullptr);
  assert(CopyDefaultRet->SourceBlock == 7);
  assert(CopyCaseRet->SourceBlock == 9);
  assert(CopyDefaultRet->Terminator == TerminatorKind::Return);
  assert(CopyCaseRet->Terminator == TerminatorKind::Return);
  assert(hasSinglePayload(CopyDefaultRet->Statements, 70));
...
void testReturnDuplicatorLowCopiesSwitchWithDiamondReturnTail() {
  ...
  const CFGBlock *CopyDiamondHead = Cfg.getBlock(CopySwitch->Cases.front().Target);
  assert(CopyDefaultTail != nullptr && CopyDiamondHead != nullptr);
  assert(CopyDefaultTail->SourceBlock == 3);
  assert(CopyDiamondHead->SourceBlock == 5);
  assert(CopyDiamondHead->Terminator == TerminatorKind::Branch);
  assert(CopyDiamondHead->Condition.Id == 50);
...
void testReturnDuplicatorLowCopiesPrefixedSwitchReturnRegion() {
  ...
  CFGBlock Switch = switchBlock(3, {4, 6});
  Switch.Condition = {30};
  Switch.Cases.clear();
  Switch.Cases.push_back({{31}, 6});
  Switch.Statements.push_back({32});
  Cfg.addBlock(std::move(Switch));

  CFGBlock DefaultTail = block(4, {5});
  DefaultTail.Statements.push_back({40});
  Cfg.addBlock(std::move(DefaultTail));

  CFGBlock DefaultRet = block(5, {});
  DefaultRet.Terminator = TerminatorKind::Return;
  DefaultRet.Statements.push_back({50});
  Cfg.addBlock(std::move(DefaultRet));

  CFGBlock CaseTail = block(6, {7});
  CaseTail.Statements.push_back({60});
  Cfg.addBlock(std::move(CaseTail));

  CFGBlock CaseRet = block(7, {});
  CaseRet.Terminator = TerminatorKind::Return;
  CaseRet.Statements.push_back({70});
  Cfg.addBlock(std::move(CaseRet));
  ...
  const CFGBlock *CopySwitch = Cfg.getBlock(CopyPrefix->Successors.front());
  assert(CopySwitch != nullptr);
  assert(CopySwitch->SourceBlock == 3);
  assert(CopySwitch->Terminator == TerminatorKind::Switch);
  assert(CopySwitch->Condition.Id == 30);
  assert(CopySwitch->Cases.size() == 1);
  assert(CopySwitch->Cases.front().Value.Id == 31);
  assert(CopySwitch->Successors.size() == 2);
  assert(CopySwitch->Successors[1] == CopySwitch->Cases.front().Target);
  assert(hasSinglePayload(CopySwitch->Statements, 32));

  const CFGBlock *CopyDefaultTail = Cfg.getBlock(CopySwitch->Successors[0]);
  const CFGBlock *CopyCaseTail = Cfg.getBlock(CopySwitch->Cases.front().Target);
  assert(CopyDefaultTail != nullptr && CopyCaseTail != nullptr);
  assert(hasSinglePayload(CopyDefaultTail->Statements, 40));
  assert(hasSinglePayload(CopyCaseTail->Statements, 60));
  ...
  assert(CopyDefaultRet->Terminator == TerminatorKind::Return);
  assert(CopyCaseRet->Terminator == TerminatorKind::Return);
  assert(hasSinglePayload(CopyDefaultRet->Statements, 50));
  assert(hasSinglePayload(CopyCaseRet->Statements, 70));
}
...
void testReturnDeduplicatorSharesLinearBranchReturnChains() {
  StructuredCFG Cfg;
  Cfg.addBlock(branchBlock(0, {1, 3}));

  CFGBlock ThenHead = block(1, {2});
  ThenHead.Statements.push_back({101});
  Cfg.addBlock(std::move(ThenHead));

  CFGBlock ThenRet = block(2, {});
  ThenRet.Statements.push_back({201});
  ThenRet.Terminator = TerminatorKind::Return;
  Cfg.addBlock(std::move(ThenRet));

  CFGBlock ElseHead = block(3, {4});
  ElseHead.Statements.push_back({102});
  Cfg.addBlock(std::move(ElseHead));

  CFGBlock ElseRet = block(4, {});
  ElseRet.Statements.push_back({301});
  ElseRet.Terminator = TerminatorKind::Return;
  Cfg.addBlock(std::move(ElseRet));
  Cfg.setPayloadOrigin(301, 201);
  ...
  assert(Changed);
  const CFGBlock *ThenHeadAfter = Cfg.getBlock(1);
  const CFGBlock *ThenRetAfter = Cfg.getBlock(2);
  const CFGBlock *ElseHeadAfter = Cfg.getBlock(3);
  const CFGBlock *ElseRetAfter = Cfg.getBlock(4);
  assert(ThenHeadAfter != nullptr && ThenRetAfter != nullptr &&
         ElseHeadAfter != nullptr && ElseRetAfter != nullptr);
  assert(ThenHeadAfter->Successors == std::vector<BlockId>{2});
  assert(ElseHeadAfter->Successors == std::vector<BlockId>{4});
  assert(hasSinglePayload(ThenHeadAfter->Statements, 101));
  assert(hasSinglePayload(ElseHeadAfter->Statements, 102));
  assert(ThenRetAfter->Terminator == TerminatorKind::Fallthrough);
  assert(ElseRetAfter->Terminator == TerminatorKind::Fallthrough);
  assert(ThenRetAfter->Statements.empty());
  assert(ElseRetAfter->Statements.empty());
  assert(ThenRetAfter->Successors.size() == 1);
  assert(ThenRetAfter->Successors == ElseRetAfter->Successors);

  const CFGBlock *SharedReturn =
      Cfg.getBlock(ThenRetAfter->Successors.front());
  assert(SharedReturn != nullptr);
  assert(SharedReturn->Origin == CFGBlockOrigin::Synthetic);
  assert(SharedReturn->CreatedBy == CFGBlockCreator::SAILRDeoptimization);
  assert(SharedReturn->Terminator == TerminatorKind::Return);
...
void testReturnDeduplicatorSharesDuplicateSwitchCaseReturns() {
  StructuredCFG Cfg;
  Cfg.addBlock(switchBlock(0, {3, 1, 2}));

  CFGBlock Case1 = block(1, {});
  Case1.Statements.push_back({101});
  Case1.Statements.push_back({201});
  Case1.Terminator = TerminatorKind::Return;
  Cfg.addBlock(std::move(Case1));

  CFGBlock Case2 = block(2, {});
  Case2.Statements.push_back({102});
  Case2.Statements.push_back({301});
  Case2.Terminator = TerminatorKind::Return;
  Cfg.addBlock(std::move(Case2));
  Cfg.setPayloadOrigin(301, 201);

  CFGBlock Default = block(3, {});
  Default.Statements.push_back({401});
  ...
  assert(Changed);
  const CFGBlock *Case1After = Cfg.getBlock(1);
  const CFGBlock *Case2After = Cfg.getBlock(2);
  const CFGBlock *DefaultAfter = Cfg.getBlock(3);
  assert(Case1After != nullptr && Case2After != nullptr &&
         DefaultAfter != nullptr);
  assert(Case1After->Terminator == TerminatorKind::Fallthrough);
  assert(Case2After->Terminator == TerminatorKind::Fallthrough);
  assert(DefaultAfter->Terminator == TerminatorKind::Return);
  assert(hasSinglePayload(Case1After->Statements, 101));
  assert(hasSinglePayload(Case2After->Statements, 102));
  assert(hasSinglePayload(DefaultAfter->Statements, 401));
  assert(Case1After->Successors.size() == 1);
  assert(Case1After->Successors == Case2After->Successors);
  assert(Case1After->Successors.front() != 3);

  const CFGBlock *SharedReturn = Cfg.getBlock(Case1After->Successors.front());
  assert(SharedReturn != nullptr);
  assert(SharedReturn->Origin == CFGBlockOrigin::Synthetic);
  assert(SharedReturn->CreatedBy == CFGBlockCreator::SAILRDeoptimization);
  assert(SharedReturn->Terminator == TerminatorKind::Return);
...
void testLoweredSwitchSimplifierBuildsSwitchFromRangeGuardedIfChain() {
  llvm::LLVMContext Context;
  llvm::Module Module("lowered-switch-range-guard-test", Context);
  llvm::Type *I32Ty = llvm::Type::getInt32Ty(Context);
  auto *FnTy =
      llvm::FunctionType::get(I32Ty, {I32Ty}, /*isVarArg=*/false);
  auto *F = llvm::Function::Create(FnTy, llvm::GlobalValue::ExternalLinkage,
  ...
  llvm::IRBuilder<> Builder(Entry);
  llvm::Value *Guard =
      Builder.CreateICmpSLE(X, llvm::ConstantInt::get(I32Ty, 9), "guard");
  Builder.CreateCondBr(Guard, Check7, Default);
  Builder.SetInsertPoint(Check7);
  llvm::Value *Cmp7 =
      Builder.CreateICmpEQ(X, llvm::ConstantInt::get(I32Ty, 7), "cmp7");
  Builder.CreateCondBr(Cmp7, Case7, Check9);
  ...
  assert(Switch->Terminator == TerminatorKind::Switch);
  assert(Payloads[Switch->Condition.Id] == "x");
  assert(Switch->Successors == std::vector<BlockId>({5, 2, 4}));
  assert(Switch->Cases.size() == 2);
  assert(Payloads[Switch->Cases[0].Value.Id] == "7");
  assert(Switch->Cases[0].Target == 2);
  assert(Payloads[Switch->Cases[1].Value.Id] == "9");
...
void testLoweredSwitchSimplifierBuildsSwitchFromNestedRangeGuardedIfChain() {
  llvm::LLVMContext Context;
  llvm::Module Module("lowered-switch-nested-range-guard-test", Context);
  llvm::Type *I32Ty = llvm::Type::getInt32Ty(Context);
  auto *FnTy =
      llvm::FunctionType::get(I32Ty, {I32Ty}, /*isVarArg=*/false);
  auto *F = llvm::Function::Create(FnTy, llvm::GlobalValue::ExternalLinkage,
  ...
  llvm::BasicBlock *Entry = llvm::BasicBlock::Create(Context, "entry", F);
  llvm::BasicBlock *UpperGuard =
      llvm::BasicBlock::Create(Context, "upper_guard", F);
  llvm::BasicBlock *Check7 = llvm::BasicBlock::Create(Context, "check7", F);
  llvm::BasicBlock *Case7 = llvm::BasicBlock::Create(Context, "case7", F);
  llvm::BasicBlock *Check9 = llvm::BasicBlock::Create(Context, "check9", F);
  llvm::BasicBlock *Case9 = llvm::BasicBlock::Create(Context, "case9", F);
  ...
  llvm::IRBuilder<> Builder(Entry);
  llvm::Value *Lower =
      Builder.CreateICmpSGE(X, llvm::ConstantInt::get(I32Ty, 7), "lower");
  Builder.CreateCondBr(Lower, UpperGuard, Default);
  Builder.SetInsertPoint(UpperGuard);
  llvm::Value *Upper =
      Builder.CreateICmpSLE(X, llvm::ConstantInt::get(I32Ty, 9), "upper");
  Builder.CreateCondBr(Upper, Check7, Default);
  Builder.SetInsertPoint(Check7);
  llvm::Value *Cmp7 =
      Builder.CreateICmpEQ(X, llvm::ConstantInt::get(I32Ty, 7), "cmp7");
  Builder.CreateCondBr(Cmp7, Case7, Check9);
  ...
  assert(Switch->Terminator == TerminatorKind::Switch);
  assert(Payloads[Switch->Condition.Id] == "x");
  assert(Switch->Successors == std::vector<BlockId>({6, 3, 5}));
  assert(Switch->Cases.size() == 2);
  assert(Payloads[Switch->Cases[0].Value.Id] == "7");
  assert(Switch->Cases[0].Target == 3);
  assert(Payloads[Switch->Cases[1].Value.Id] == "9");
  assert(Switch->Cases[1].Target == 5);
}

void testLoweredSwitchSimplifierSkipsInnerChainWhenRangeGuardDefaultDiffers() {
  StructuredCFG Cfg;

  CFGBlock Head = branchBlock(2, {10, 3});
  Head.Condition = {100};
  Cfg.addBlock(std::move(Head));

  CFGBlock Next = branchBlock(3, {11, 30});
  Next.Condition = {101};
  ...
  Cfg.addBlock(block(11, {}));
  Cfg.addBlock(block(30, {}));
  Cfg.addBlock(block(31, {}));

  CFGBlock Guard = branchBlock(0, {2, 31});
  Guard.Condition = {102};
  ...
  assert(!Changed);
  const CFGBlock *HeadAfter = Cfg.getBlock(2);
...
void testLoweredSwitchSimplifierBuildsSwitchFromRangeTree() {
  StructuredCFG Cfg;
  ...
  CFGBlock HighNext = branchBlock(4, {13, 30});
  HighNext.Condition = {104};
  Cfg.addBlock(std::move(HighNext));

  for (BlockId Id : {10, 11, 12, 13, 30}) {
  ...
  assert(Switch->Cases[1].Value.Id == 302);
  assert(Switch->Cases[1].Target == 11);
  assert(Switch->Cases[2].Value.Id == 304);
  assert(Switch->Cases[2].Target == 12);
  assert(Switch->Cases[3].Value.Id == 306);
...
void testLoweredSwitchSimplifierSkipsRangeTreeWithDifferentDefaults() {
  ...
  for (BlockId Id : {10, 11, 12, 13, 30, 31}) {
    Cfg.addBlock(block(Id, {}));
  }
  ...
  assert(GuardAfter != nullptr);
  assert(GuardAfter->Terminator == TerminatorKind::Branch);
  assert(GuardAfter->Successors == std::vector<BlockId>({1, 3}));
}
...
void testLoweredSwitchSimplifierSkipsDefaultThatReachesIfChain() {
  StructuredCFG Cfg;
...
void testLoweredSwitchSimplifierSkipsRangeGuardWithSharedDefaultTarget() {
  StructuredCFG Cfg;

  CFGBlock Guard = branchBlock(0, {1, 20});
  Guard.Condition = {100};
  Cfg.addBlock(std::move(Guard));
  ...
  CFGBlock Next = branchBlock(2, {11, 20});
  Next.Condition = {102};
  Cfg.addBlock(std::move(Next));
  ...
  assert(GuardAfter != nullptr);
  assert(GuardAfter->Terminator == TerminatorKind::Branch);
  assert(GuardAfter->Successors == std::vector<BlockId>({1, 20}));
  assert(Cfg.getBlock(1) != nullptr);
  assert(Cfg.getBlock(2) != nullptr);
}
...
void testLoweredSwitchSimplifierSkipsRangeGuardWithOutsideCase() {
  llvm::LLVMContext Context;
  llvm::Module Module("lowered-switch-range-guard-skip-test", Context);
  llvm::Type *I32Ty = llvm::Type::getInt32Ty(Context);
  auto *FnTy =
      llvm::FunctionType::get(I32Ty, {I32Ty}, /*isVarArg=*/false);
  auto *F = llvm::Function::Create(FnTy, llvm::GlobalValue::ExternalLinkage,
  ...
  llvm::IRBuilder<> Builder(Entry);
  llvm::Value *Guard =
      Builder.CreateICmpSLE(X, llvm::ConstantInt::get(I32Ty, 8), "guard");
  Builder.CreateCondBr(Guard, Check7, Default);
  Builder.SetInsertPoint(Check7);
  llvm::Value *Cmp7 =
      Builder.CreateICmpEQ(X, llvm::ConstantInt::get(I32Ty, 7), "cmp7");
  ...
  assert(!Changed);
  const CFGBlock *GuardBlock = Cfg.getBlock(0);
  assert(GuardBlock != nullptr);
  assert(GuardBlock->Terminator == TerminatorKind::Branch);
  assert(Cfg.getBlock(1) != nullptr);
  assert(Cfg.getBlock(3) != nullptr);
}

void testLoweredSwitchSimplifierKeepsOuterRangeGuardWithDifferentDefault() {
  llvm::LLVMContext Context;
  llvm::Module Module("lowered-switch-nested-range-guard-skip-test", Context);
  llvm::Type *I32Ty = llvm::Type::getInt32Ty(Context);
  auto *FnTy =
      llvm::FunctionType::get(I32Ty, {I32Ty}, /*isVarArg=*/false);
  auto *F = llvm::Function::Create(FnTy, llvm::GlobalValue::ExternalLinkage,
  ...
  llvm::BasicBlock *Case9 = llvm::BasicBlock::Create(Context, "case9", F);
  llvm::BasicBlock *Default = llvm::BasicBlock::Create(Context, "default", F);
  llvm::BasicBlock *LowDefault =
      llvm::BasicBlock::Create(Context, "low_default", F);

  llvm::IRBuilder<> Builder(Entry);
  llvm::Value *Lower =
      Builder.CreateICmpSGE(X, llvm::ConstantInt::get(I32Ty, 7), "lower");
  Builder.CreateCondBr(Lower, UpperGuard, LowDefault);
  Builder.SetInsertPoint(UpperGuard);
  llvm::Value *Upper =
      Builder.CreateICmpSLE(X, llvm::ConstantInt::get(I32Ty, 9), "upper");
  Builder.CreateCondBr(Upper, Check7, Default);
  ...
  Builder.SetInsertPoint(Default);
  Builder.CreateRet(llvm::ConstantInt::get(I32Ty, 0));
  Builder.SetInsertPoint(LowDefault);
  Builder.CreateRet(llvm::ConstantInt::get(I32Ty, -1));
  ...
  assert(GuardBlock != nullptr);
  assert(GuardBlock->Terminator == TerminatorKind::Branch);
  assert(GuardBlock->Successors == std::vector<BlockId>({7, 1}));

  const CFGBlock *InnerSwitch = Cfg.getBlock(1);
  assert(InnerSwitch != nullptr);
  assert(InnerSwitch->Terminator == TerminatorKind::Switch);
  assert(InnerSwitch->Successors == std::vector<BlockId>({6, 3, 5}));
  assert(Cfg.getBlock(1) != nullptr);
  assert(Cfg.getBlock(2) == nullptr);
  assert(Cfg.getBlock(4) == nullptr);
}
...
int main() {
  ...
  testStructuredCFGDuplicateCopiedDephicationEdgeAfterOriginalRemoval();
  ...
  testReturnDuplicatorLowCopiesJoinedDiamondReturnRegion();
  testReturnDuplicatorLowCopiesJoinedDiamondWithDirectJoinSide();
  testReturnDuplicatorLowSkipsAllDirectJoinedDiamond();
  ...
  testReturnDeduplicatorSharesLinearBranchReturnChains();
  ...
  testLoweredSwitchSimplifierSkipsRangeGuardWithOutsideCase();
  testLoweredSwitchSimplifierKeepsOuterRangeGuardWithDifferentDefault();
